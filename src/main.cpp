...
void setCursorForDirection(Direction dir) {
    ...
    switch (dir) {
        case UP:
            cursor = LoadCursor(NULL, MAKEINTRESOURCE(32655)); break;
        case DOWN:
		 	cursor = LoadCursor(NULL, MAKEINTRESOURCE(32656)); break;
        case RIGHT:
		 	cursor = LoadCursor(NULL, MAKEINTRESOURCE(32657)); break;
        case LEFT:
            cursor = LoadCursor(NULL, MAKEINTRESOURCE(32658)); break;
        case NONE:
        default:
            cursor = LoadCursor(NULL, MAKEINTRESOURCE(32654)); break; 
    }
...
class $modify(TableViewMouseScroll,TableView)
{
	...
	void ForceScrollUpdate(float dt)
	{
		...
		if (CurrentStagedInput == m_fields->currentInput) goto Updated;
		...
		if (CurrentStagedInput || m_fields->m_movement)
		{
			...
			{
				if (!PositionOverlap(mousePos)) return;
